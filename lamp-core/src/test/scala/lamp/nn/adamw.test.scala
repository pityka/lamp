...
class AdamWSuite extends AnyFunSuite {
  ...
  test1("AdamW without weight decay") { device =>
    Scope.root { implicit scope =>
      ...
      val params = lamp.saddle.fromMat(initParams, device)
      val gradients = lamp.saddle.fromMat(Mat(Vec(0.5, 0.75)).T, device)
      val opt = AdamW(
      ...
      assert(
        (updatedParams1 - Mat(Vec(0.9000000063245549, 0.90000000421637)).T).toVec.map(math.abs).mean2 < 1e-4
      )
      opt.step(List((gradients)), 1d)
      val updatedParams2 = params.toMat

      assert(
        (updatedParams2 - Mat(Vec(0.8000000109128679, 0.8000000072752449)).T).toVec.map(math.abs).mean2 < 1e-4
      )
    }
  }
  test1("RAdam without weight decay") { cuda =>
    Scope.root { implicit scope =>
      ...
      val updatedParams1 = params.toMat
      assert(
        (updatedParams1 - Mat(Vec(0.95, 0.925)).T).toVec.map(math.abs).mean2 < 1e-4
      )
      opt.step(List((gradients)), 1d)
      val updatedParams2 = params.toMat

      assert(
        (updatedParams2 - Mat(Vec(0.8999999999999992, 0.849999999999999)).T).toVec.map(math.abs).mean2 < 1e-4
      )
    }
  }
  test1("AdamW") { cuda =>
    Scope.root { implicit scope =>
      ...
      assert(
        ...
        ).T).toVec.map(math.abs).mean2 < 1e-4
      )
      opt.step(List((gradients)), 1d)
      val updatedParams2 = params.toMat
