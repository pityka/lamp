  ...
  test("mnist tabular mini batch - data parallel cpu") {
    Scope.root { implicit scope =>
      ...
      val model2 = SupervisedModel(
        ...
        LossFunctions.NLL(10, device.to(classWeights))
      )
      val model3 = SupervisedModel(
        logisticRegression(
          data.numCols - 1,
          10,
          device.options(DoublePrecision)
      ...
      val (_, trainedModel, _, _, _) = IOLoops
        .epochs(
          ...
          trainBatchesOverEpoch =
            _ => BatchStream.minibatchesFromFull(2000, true, x, target, rng),
          validationBatchesOverEpoch = Some((_: IOLoops.TrainingLoopContext) =>
            BatchStream.minibatchesFromFull(2000, true, x, target, rng)
          ),
          epochs = 1000,
          trainingCallback = None,
          validationCallback = None,
          dataParallelModels = List(model2, model3),
