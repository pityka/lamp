...
class KmeansSuite extends AnyFunSuite {
  ...
  test1("simple") { cuda =>
    Scope.unsafe { implicit scope =>
      ...
      val c2 =
        STen.randn(List(100, 2)) * 0.1 + lamp.saddle.fromMat(
          Mat(Vec(100d, 100d)).T
        )
      val c3 =
        STen.randn(List(100, 2)) * 0.1 + lamp.saddle.fromMat(
      ...
      0 until 100 foreach { _ =>
        Scope { implicit scope =>
          ...
          val (members, distances) =
            assignInstances(instances = d2, centers = centers)
          ...
          def assertCenter(c: Vec[Double], min: Int, max: Int) = {
            ...
            assert(p.isDefined)
            assert(
              members.toLongVec.slice(min, max).toSeq.forall(_ == p.get._2)
            )
