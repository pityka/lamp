...
class KnnSuite extends AnyFunSuite {
  ...
  test1("euclidean") { cuda =>
    ...
    val t = TensorHelpers.fromMat(m3x2, cuda)
    val d = TensorHelpers.toMat(squaredEuclideanDistance(t, t))
  ...
  test1("jaccard") { cuda =>
    ...
    val t = TensorHelpers.fromMat(m2x2, cuda)
    val d = TensorHelpers.toMat(jaccardDistance(t, t))
  ...
  test1("knnSearch") { cuda =>
    val data = Mat(Vec(0d, 0d), Vec(50d, 50d), Vec(1000d, 1000d), Vec(1d, 1d)).T
