  ...
  OParser.parse(parser1, args, CliConfig()) match {
    case Some(config) =>
      ...
      Scope.root { implicit scope =>
        val devices =
          if (config.gpus.isEmpty) List(CPU)
          else config.gpus.map(CudaDevice.apply)
        ...
        val batchStream = testEpochs(IOLoops.TrainingLoopContext(0, None, None))

        batchStream
          .allocateBuffers(device)
          .use { buffers =>
            batchStream
              .nextBatch(device, buffers, 0)
              .flatMap(
                _._2
                  .use {
